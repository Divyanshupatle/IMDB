...
export default function WatchList({ movieList, setMovieList, removeFromWatchList }) {
      ...
      <div className="flex flex-wrap m-8">
        {genre.map((item, index) => (
          <div key={index}
            className={
              item == currGenre ?
                "mx-3 flex justify-center items-center bg-blue-400 h-[3rem] w-[8rem] text-white font-bold border rounded-xl cursor-pointer mb-1"
                : " mx-3 flex justify-center items-center h-[3rem] w-[8rem] bg-gray-400/50 rounded-xl text-white font-bold cursor-pointer mb-1"}
            onClick={() => { setCurrGenre(item) }}>{item}</div>
        ))}
      </div>
          ...
          <tbody className="divide-y divide-gray-200">
            {movieList && movieList
              ...
              .filter((movie) => {
                ...
                if (genreids[movie.genre_ids[0]] == currGenre) return true;
                else return false;
              })
              .map((movie, idx) => (
                <tr
